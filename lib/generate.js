  ...
  return new Promise((resolve, reject) => {
    ...
    image.metadata((err, metadata) => {
      ...
      if (width || height) {
        if (config.withoutEnlargement && (width > metadata.width || height > metadata.height)) {
          ...
          if (options.errorOnEnlargement) {
            ...
            if (!options.silent) {
              debug('%s : (skip for processing)', msgPrefix)
            }
          ...
          if (!options.silent) {
            debug('%s : (skip for enlargement)', msgPrefix)
          }
        }
      }
      ...
      try {
        ...
        /*  use the right cropping strategy http://sharp.pixelplumbing.com/en/v0.22.1/api-resize/#resize
         */
        ...
        /* quality, progessive, withoutChromaSubsampling, compressionLevel methods
         were deprecated https://sharp.pixelplumbing.com/en/stable/changelog/#v0170-11th-december-2016
         */
